...
/**
 * Class FilterWithRange
 *
 * @package lenal\catalog\Helpers\FilterTypes
 */
class FilterWithRange extends FilterType
{
    /**
     * @var Builder
     */
    ...
    public function filter()
    {
        if (is_array($this->request_range) && isset($this->request_range['min'], $this->request_range['max'])) {
            $this->model_builder->whereBetween(
                $this->filter_key,
                [
                    $this->request_range['min'],
                    $this->request_range['max'],
                ]
            );
