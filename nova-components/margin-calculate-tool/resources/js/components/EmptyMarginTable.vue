<template>
    <div>
        <card
                v-if="selectedMargins.length === 0 && !caratRangeIsValid"
        ...
        >
            ...
            <h2 class="text-white text-4xl text-90 font-light mb-6">
                {{ getTranslation('default_empty_values') }}
            </h2>

            <button
                    class="btn btn-default btn-primary"
                    @click="createCaratRange"
            >
                {{ getTranslation('add_margin') }}
            </button>
        </card>

        <margin-table
            v-if="caratRangeIsValid"
            :caratMin="caratMin"
            :caratMax="caratMax"
        />
    </div>
...
<script>
  ...
  import caratRange from '../mixins/caratRange';

  import marginTable from './MarginTable.vue';

  export default {
    name: "empty-margin-table",
